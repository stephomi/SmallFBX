...
template <class T> class RawVector {
  public:
    ...
    template <class U> void assign(span<U> v) {
        size_t n = v.size();
        resize(n);
        for (size_t i = 0; i < n; ++i)
            m_data[i] = T(v[i]);
    }
    template <class U> void assign(const U *v, size_t n) { assign(make_span(v, n)); }

    template <class ForwardIter> void insert(iterator pos, ForwardIter first, ForwardIter last) {
    ...
    void insert(iterator pos, const_reference v) { insert(pos, &v, &v + 1); }
    ...
    bool operator!=(const RawVector &v) const { return !(*this == v); }

    void zeroclear() { memset(m_data, 0, sizeof(T) * m_size); }
