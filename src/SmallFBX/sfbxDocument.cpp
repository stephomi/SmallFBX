...
static const uint8_t g_fbx_file_id[16]{0x28, 0xb3, 0x2a, 0xeb, 0xb6, 0x24, 0xcc, 0xc2,
                                       0xbf, 0xc8, 0xb0, 0x2a, 0xa9, 0x2b, 0xfc, 0xf1};
static const uint8_t g_fbx_footer_magic1[16]{0xfa, 0xbc, 0xab, 0x09, 0xd0, 0xc8, 0xd4, 0x66,
                                             0xb1, 0x76, 0xfb, 0x83, 0x1c, 0xf7, 0x26, 0x7e};
static const uint8_t g_fbx_footer_magic2[16]{0xf8, 0x5a, 0x8c, 0x6a, 0xde, 0xf5, 0xd9, 0x7e,
                                             0xec, 0xe9, 0x0c, 0xe3, 0x75, 0x8f, 0x29, 0x0b};

Document::Document() { initialize(); }

bool Document::valid() const { return !m_objects.empty(); }
...
bool Document::writeAscii(std::ostream &os) const {
    ...
    for (auto node : getRootNodes()) {
        ...
        if (node->getName() == sfbxS_FileId || node->getName() == sfbxS_CreationTime ||
...
bool Document::writeAscii(const std::string &path) const {
    std::ofstream file;
    file.open(path, std::ios::out | std::ios::binary);
    if (file) return writeAscii(file);
    return false;
}
...
FileVersion Document::getFileVersion() const { return m_version; }

void Document::setFileVersion(FileVersion v) { m_version = v; }
...
Node *Document::findNode(string_view name) const {
    auto it = std::find_if(m_nodes.begin(), m_nodes.end(), [&name](const NodePtr &p) { return p->getName() == name; });
...
void Document::createLinkOO(Object *child, Object *parent) {
    ...
    if (auto c = findNode(sfbxS_Connections)) c->createChild(sfbxS_C, sfbxS_OO, child->getID(), parent->getID());
}
...
Object *Document::createObject(ObjectClass c, ObjectSubClass s) {
    ...
    case ObjectClass::NodeAttribute:
        switch (s) {
        case ObjectSubClass::Null:
            r = new NullAttribute();
            break;
        case ObjectSubClass::Root:
            r = new RootAttribute();
            break;
        case ObjectSubClass::LimbNode:
            r = new LimbNodeAttribute();
            break;
        case ObjectSubClass::Light:
            r = new LightAttribute();
            break;
        case ObjectSubClass::Camera:
            r = new CameraAttribute();
            break;
        default:
            r = new NodeAttribute();
            break;
    ...
    case ObjectClass::Deformer:
        switch (s) {
        case ObjectSubClass::Skin:
            r = new Skin();
            break;
        case ObjectSubClass::Cluster:
            r = new Cluster();
            break;
        case ObjectSubClass::BlendShape:
            r = new BlendShape();
            break;
        case ObjectSubClass::BlendShapeChannel:
            r = new BlendShapeChannel();
            break;
        default:
            r = new Deformer();
            break;
    ...
    case ObjectClass::Pose:
        ...
        break;
    case ObjectClass::Video:
        r = new Video();
        break;
    case ObjectClass::Texture:
        r = new Texture();
        break;
    ...
    case ObjectClass::AnimationStack:
        r = new AnimationStack();
        break;
    case ObjectClass::AnimationLayer:
        r = new AnimationLayer();
        break;
    case ObjectClass::AnimationCurveNode:
        r = new AnimationCurveNode();
        break;
    case ObjectClass::AnimationCurve:
        r = new AnimationCurve();
        break;
    case ObjectClass::Implementation:
        r = new Implementation();
        break;
    case ObjectClass::BindingTable:
        r = new BindingTable();
        break;
    default:
        break;
...
template <class T> T *Document::createObject(string_view name) {
    T *r = new T();
    r->setName(name);
    addObject(ObjectPtr(r));
    return r;
...
#undef Body
    ...
    if (obj) {
        if (check && find(m_objects, obj)) return;
        m_objects.push_back(obj);
        if (auto take = as<AnimationStack>(obj.get())) m_anim_stacks.push_back(take);
        obj->m_document = this;
    }
}
...
Object *Document::findObject(string_view name) const { return FindObjectByName(m_objects, name); }
...
span<AnimationStack *> Document::getAnimationStacks() const { return make_span(m_anim_stacks); }

AnimationStack *Document::findAnimationStack(string_view name) const { return FindObjectByName(m_anim_stacks, name); }
...
bool Document::mergeAnimations(Document *doc) {
    if (!doc || !doc->valid()) return false;

    size_t num_merged = 0;
    for (auto take : doc->getAnimationStacks()) {
        if (take->remap(this)) ++num_merged;
    }
    if (!m_current_take && !m_anim_stacks.empty()) m_current_take = m_anim_stacks.front();
    return num_merged != 0;
}
...
void Document::exportFBXNodes() {
    ...
    auto header_extension = createNode(sfbxS_FBXHeaderExtension);
    {
        ...
        {
            auto scene_info = header_extension->createChild(
                sfbxS_SceneInfo, MakeFullName(sfbxS_GlobalInfo, sfbxS_SceneInfo), sfbxS_UserData);
    ...
    auto gsettings = createNode(sfbxS_GlobalSettings);
    {
        gsettings->createChild(sfbxS_Version, sfbxI_GlobalSettingsVersion);
        auto prop = gsettings->createChild(sfbxS_Properties70);
        ...
        prop->createChild(sfbxS_P, sfbxS_UnitScaleFactor, sfbxS_double, sfbxS_Number, "",
                          this->global_settings.unit_scale);
        prop->createChild(sfbxS_P, sfbxS_OriginalUnitScaleFactor, sfbxS_double, sfbxS_Number, "",
                          this->global_settings.unit_scale);
    ...
    for (auto *t : m_anim_stacks) {
        ...
        if (lstart != 0.0f || lstop != 0.0f) take->createChild(sfbxS_LocalTime, ToTicks(lstart), ToTicks(lstop));
        if (rstart != 0.0f || rstop != 0.0f) take->createChild(sfbxS_ReferenceTime, ToTicks(rstart), ToTicks(rstop));
    }
}
