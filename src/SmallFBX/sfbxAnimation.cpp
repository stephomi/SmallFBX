...
void AnimationStack::addChild(Object *v) {
    super::addChild(v);
    if (auto l = as<AnimationLayer>(v)) m_anim_layers.push_back(l);
}

void AnimationStack::eraseChild(Object *v) {
    super::eraseChild(v);
    if (auto l = as<AnimationLayer>(v)) erase(m_anim_layers, l);
}
...
AnimationLayer *AnimationStack::createLayer(string_view name) { return createChild<AnimationLayer>(name); }
...
bool AnimationStack::remap(Document *doc) {
    ...
    for (auto layer : make_reverse(getAnimationLayers())) {
        if (!layer->remap(doc)) eraseChild(layer);
    }
    if (m_anim_layers.empty()) return false;
...
void AnimationLayer::exportFBXObjects() { super::exportFBXObjects(); }

void AnimationLayer::addChild(Object *v) {
    super::addChild(v);
    if (auto acn = as<AnimationCurveNode>(v)) m_anim_nodes.push_back(acn);
}

void AnimationLayer::eraseChild(Object *v) {
    super::eraseChild(v);
    if (auto acn = as<AnimationCurveNode>(v)) erase(m_anim_nodes, acn);
}

span<AnimationCurveNode *> AnimationLayer::getAnimationCurveNodes() const { return make_span(m_anim_nodes); }
...
void AnimationLayer::merge(AnimationLayer *src) {
    ...
    for (auto node : make_reverse(src->getAnimationCurveNodes())) {
        if (auto old = find_if(m_anim_nodes, [node](auto n) {
                return node->getAnimationTarget() == n->getAnimationTarget() &&
                       node->getAnimationKind() == n->getAnimationKind();
            })) {
            old->unlink();
        }
        src->eraseChild(node);
...
static const AnimationCurveInfo *FindAnimationCurveInfo(string_view name) {
    for (auto &info : g_acinfo)
        if (info.link_name == name) return &info;
    return nullptr;
}
...
void AnimationCurveNode::exportFBXObjects() {
    ...
    auto props = getNode()->createChild(sfbxS_Properties70);
    if (FindAnimationKindInfo(getName())) {
...
void AnimationCurveNode::exportFBXConnections() {
    ...
    if (auto *info = FindAnimationKindInfo(m_kind)) {
        if (auto *target = getAnimationTarget()) m_document->createLinkOP(this, target, info->link_name);
        for (auto curve : m_curves)
...
void AnimationCurveNode::addChild(Object *v) {
    super::addChild(v);
    if (auto curve = as<AnimationCurve>(v)) m_curves.push_back(curve);
}
...
void AnimationCurveNode::eraseChild(Object *v) {
    super::eraseChild(v);
    if (auto curve = as<AnimationCurve>(v)) erase(m_curves, curve);
}

AnimationKind AnimationCurveNode::getAnimationKind() const { return m_kind; }

Object *AnimationCurveNode::getAnimationTarget() const {
    for (auto p : getParents())
        if (!as<AnimationLayer>(p)) return p;
    return nullptr;
}

span<AnimationCurve *> AnimationCurveNode::getAnimationCurves() const { return make_span(m_curves); }
...
float AnimationCurveNode::evaluateF1(float time) const {
    if (m_curves.empty()) return m_default_value.f3.x;
...
int AnimationCurveNode::evaluateI(float time) const {
    if (m_curves.empty()) return m_default_value.i;
    return (int)m_curves[0]->evaluate(time);
}

void AnimationCurveNode::applyAnimation(float time) const {
    if (m_curves.empty() || m_kind == AnimationKind::Unknown) return;
...
void AnimationCurveNode::setup(AnimationKind kind, Object *target, bool create_curves) {
    ...
    if (kind >= AnimationKind::Position && kind <= AnimationKind::Scale) { // transform
        ...
        if (auto attr = as<LightAttribute>(target)) m_target.light = as<Light>(attr->getParent());
    } else if (kind >= AnimationKind::FocalLength && kind <= AnimationKind::FilmOffsetY) { // camera
        if (auto attr = as<CameraAttribute>(target)) m_target.camera = as<Camera>(attr->getParent());
...
bool AnimationCurveNode::remap(Document *doc) {
    if (m_kind == AnimationKind::Unknown || m_curves.empty()) return false;
    for (auto &p : getParents()) {
