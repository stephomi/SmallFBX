...
GeomMesh *Deformer::getBaseMesh() const {
    for (auto *p = getParent(); p; p = p->getParent())
        if (auto geom = as<GeomMesh>(p)) return geom;
    return nullptr;
}

void Deformer::deformPoints(span<float3>) const {}
void Deformer::deformNormals(span<float3>) const {}
...
void Skin::addParent(Object *v) {
    super::addParent(v);
    if (auto mesh = as<GeomMesh>(v)) m_mesh = mesh;
}
...
void Skin::eraseChild(Object *v) {
    super::eraseChild(v);
    if (auto cluster = as<Cluster>(v)) erase(m_clusters, cluster);
}
...
const JointWeights &Skin::getJointWeights() const {
    auto &ret = m_weights;
    if (!ret.counts.empty()) return ret;
...
JointWeights Skin::createFixedJointWeights(int joints_per_vertex) const {
    ...
    if (tmp.weights.empty()) return tmp;
...
void Cluster::exportFBXObjects() {
    ...
    if (!m_weights.empty()) n->createChild(sfbxS_Weights, make_adaptor<float64>(m_weights));
    if (m_transform != float4x4::identity()) n->createChild(sfbxS_Transform, (double4x4)m_transform);
    if (m_transform_link != float4x4::identity()) n->createChild(sfbxS_TransformLink, (double4x4)m_transform_link);
}
...
void BlendShape::addChild(Object *v) {
    super::addChild(v);
    if (auto ch = as<BlendShapeChannel>(v)) m_channels.push_back(ch);
}

void BlendShape::eraseChild(Object *v) {
    super::eraseChild(v);
    if (auto ch = as<BlendShapeChannel>(v)) erase(m_channels, ch);
}

span<BlendShapeChannel *> BlendShape::getChannels() const { return make_span(m_channels); }

BlendShapeChannel *BlendShape::createChannel(string_view name) { return createChild<BlendShapeChannel>(name); }
...
void BlendShapeChannel::exportFBXObjects() {
    ...
    auto props = n->createChild(sfbxS_Properties70);
    props->createChild(sfbxS_P, sfbxS_DeformPercent, sfbxS_Number, "", "A", (float64)(m_weight * WeightToPercent));
...
float BlendShapeChannel::getWeight() const { return m_weight; }

span<BlendShapeChannel::ShapeData> BlendShapeChannel::getShapeData() const { return make_span(m_shape_data); }
...
void BlendShapeChannel::setWeight(float v) { m_weight = v; }

void BlendShapeChannel::deformPoints(span<float3> dst) const {
    if (m_weight == 0.0f || m_shape_data.empty()) return;
