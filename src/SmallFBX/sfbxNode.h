...
class Node {
  ...
  public:
    ...
    template <class... T> void addProperties(T &&...v) {
        reserveProperties(getProperties().size() + sizeof...(T));
        addProperties_(v...);
    }
    template <class... T> Node *createChild(string_view name, T &&...v) {
        auto r = createChild(name);
        r->addProperties(v...);
        return r;
    }
  ...
  private:
    void addProperties_() {}
    template <class T, class... U> void addProperties_(T &&v, U &&...a) {
        addProperty(v);
        addProperties_(a...);
    }
