...
template <class T> class span {
  public:
    ...
    template <size_t N> span(const T (&v)[N]) : m_data(const_cast<T *>(v)), m_size(N) {}
...
template <class T, size_t N> struct array_to_span {
    span<T> operator()(const T (&v)[N]) const { return {const_cast<T *>(v), N}; }
};
template <size_t N> struct array_to_span<char, N> {
    span<char> operator()(const char (&v)[N]) const {
        return {const_cast<char *>(v), N - 1};
    } // -1 to ignore null terminator
};
// from array
template <class T, size_t N> inline constexpr span<T> make_span(const T (&v)[N]) { return array_to_span<T, N>()(v); }

// from contagious container (std::vector, etc)
template <class Cont, sfbxRestrict(is_contiguous_container<Cont>)>
inline constexpr span<get_value_type<Cont>> make_span(const Cont &v) {
    return {const_cast<get_value_type<Cont> *>(v.data()), v.size()};
}

// from pointer & size
template <class T> inline constexpr span<T> make_span(const T *v, size_t n) { return {const_cast<T *>(v), n}; }

template <class T> class reverse_span {
  public:
    struct iterator {
        T *m_ptr;
...
template <class T> inline reverse_span<T> make_reverse(span<T> v) { return reverse_span<T>{v}; }

using std::string_view;

template <size_t N> inline constexpr string_view make_view(const char (&v)[N]) {
    return string_view{v, N - 1};
} // -1 to ignore null terminator

template <class Cont, sfbxRestrict(is_contiguous_container<Cont>)>
inline constexpr string_view make_view(const Cont &v) {
    return {const_cast<get_value_type<Cont> *>(v.data()), v.size()};
}

inline constexpr string_view make_view(const char *v, size_t n) { return {v, n}; }
inline constexpr string_view make_view(const char *begin, const char *end) {
    return make_view(begin, (size_t)std::distance(begin, end));
}
...
template <class T> struct get_scalar<tvec2<T>> {
    using type = T;
};
template <class T> struct get_scalar<tvec3<T>> {
    using type = T;
};
template <class T> struct get_scalar<tvec4<T>> {
    using type = T;
};
template <class T> struct get_scalar<tquat<T>> {
    using type = T;
};
template <class T> struct get_scalar<tmat4x4<T>> {
    using type = T;
};
...
enum class RotationOrder : int { XYZ, XZY, YZX, YXZ, ZXY, ZYX, SphericXYZ };
...
template <class D, class S> struct array_adaptor {
    using dst_type = D;
    using src_type = S;
...
template <class D, class S, class... T> inline auto make_adaptor(const S &src, T &&...a) {
    return array_adaptor<D, get_value_type<S>>(src, a...);
}
...
template <class T, class U> inline T *as(U *v) { return dynamic_cast<T *>(v); }
