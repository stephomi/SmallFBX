...
template <class T> inline constexpr bool is_deformer = std::is_base_of_v<Deformer, T>;
...
class Geometry : public Object {
  ...
  public:
    ...
    // T: Skin, BlendShape
    template <class T, sfbxRestrict(is_deformer<T>)> T *createDeformer();
...
template <class T> struct LayerElement {
    std::string name;
...
class GeomMesh : public Geometry {
  ...
  public:
    ...
    span<LayerElementI1> getPolygonGroupLayers() const;    // can be zero or multiple layers
    ...
    void addPolygonGroupLayer(LayerElementI1 &&v);
  ...
  protected:
    ...
    std::vector<LayerElementI1> m_polygonGroup_layers;
