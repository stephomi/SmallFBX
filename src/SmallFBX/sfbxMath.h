...
template <class T, class U> inline tvec2<T> operator+(const tvec2<T> &l, const tvec2<U> &r) {
...
template <class T> inline tvec2<T> operator*(const tvec2<T> &l, T r) { return {l.x * r, l.y * r}; }
template <class T> inline tvec2<T> operator/(const tvec2<T> &l, T r) { return {l.x / r, l.y / r}; }
template <class T, class U> inline tvec2<T> &operator+=(tvec2<T> &l, const tvec2<U> &r) {
    l.x += r.x;
    l.y += r.y;
    return l;
}
...
template <class T> inline tvec2<T> &operator+=(tvec2<T> &l, T r) {
    l.x += r;
    l.y += r;
    return l;
}
...
template <class T, class U> inline tvec3<T> operator+(const tvec3<T> &l, const tvec3<U> &r) {
...
template <class T> inline tvec3<T> operator*(const tvec3<T> &l, T r) { return {l.x * r, l.y * r, l.z * r}; }
template <class T> inline tvec3<T> operator/(const tvec3<T> &l, T r) { return {l.x / r, l.y / r, l.z / r}; }
template <class T, class U> inline tvec3<T> &operator+=(tvec3<T> &l, const tvec3<U> &r) {
    l.x += r.x;
    l.y += r.y;
    l.z += r.z;
    return l;
...
template <class T> inline tvec3<T> &operator+=(tvec3<T> &l, T r) {
    l.x += r;
    l.y += r;
    l.z += r;
    return l;
...
template <class T, class U> inline tvec4<T> operator+(const tvec4<T> &l, const tvec4<U> &r) {
...
template <class T> inline tvec4<T> operator*(const tvec4<T> &l, T r) { return {l.x * r, l.y * r, l.z * r, l.w * r}; }
template <class T> inline tvec4<T> operator/(const tvec4<T> &l, T r) { return {l.x / r, l.y / r, l.z / r, l.w / r}; }
template <class T, class U> inline tvec4<T> &operator+=(tvec4<T> &l, const tvec4<U> &r) {
    l.x += r.x;
    l.y += r.y;
    l.z += r.z;
    l.w += r.w;
...
template <class T> inline tvec4<T> &operator+=(tvec4<T> &l, T r) {
    l.x += r;
    l.y += r;
    l.z += r;
    l.w += r;
...
template <class T> inline T dot(const tvec4<T> &l, const tvec4<T> &r) {
    return l.x * r.x + l.y * r.y + l.z * r.z + l.w * r.w;
}
template <class T> inline T dot(const tquat<T> &l, const tquat<T> &r) {
    return dot((const tvec4<T> &)l, (const tvec4<T> &)r);
}
...
template <class T> inline tquat<T> rotate_euler(RotationOrder order, const tvec3<T> &euler) {
    ...
    case RotationOrder::ZYX:
        return rx * ry * rz;
    case RotationOrder::SphericXYZ:
        return rz * ry * rx; // todo
    default:
...
template <class T> inline tmat4x4<T> to_mat4x4(const tquat<T> &v) {
    return tmat4x4<T>{{{T(1.0) - T(2.0) * v.y * v.y - T(2.0) * v.z * v.z, T(2.0) * v.x * v.y - T(2.0) * v.z * v.w,
                        T(2.0) * v.x * v.z + T(2.0) * v.y * v.w, T(0.0)},
                       {T(2.0) * v.x * v.y + T(2.0) * v.z * v.w, T(1.0) - T(2.0) * v.x * v.x - T(2.0) * v.z * v.z,
                        T(2.0) * v.y * v.z - T(2.0) * v.x * v.w, T(0.0)},
                       {T(2.0) * v.x * v.z - T(2.0) * v.y * v.w, T(2.0) * v.y * v.z + T(2.0) * v.x * v.w,
                        T(1.0) - T(2.0) * v.x * v.x - T(2.0) * v.y * v.y, T(0.0)},
                       {T(0.0), T(0.0), T(0.0), T(1.0)}}};
}

template <class T> inline tmat4x4<T> translate(const tvec3<T> &v) {
    return tmat4x4<T>{{{T(1.0), T(0.0), T(0.0), T(0.0)},
...
template <class T> inline tmat4x4<T> invert(const tmat4x4<T> &x) {
    tmat4x4<T> s = {{
        {x[1][1] * x[2][2] - x[2][1] * x[1][2], x[2][1] * x[0][2] - x[0][1] * x[2][2],
         x[0][1] * x[1][2] - x[1][1] * x[0][2], 0},

        {x[2][0] * x[1][2] - x[1][0] * x[2][2], x[0][0] * x[2][2] - x[2][0] * x[0][2],
         x[1][0] * x[0][2] - x[0][0] * x[1][2], 0},

        {x[1][0] * x[2][1] - x[2][0] * x[1][1], x[2][0] * x[0][1] - x[0][0] * x[2][1],
         x[0][0] * x[1][1] - x[1][0] * x[0][1], 0},
